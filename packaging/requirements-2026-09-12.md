...
| --- | --- |
| `get-started` | account ambiguity: `account_context_get` (`operation_catalog/account_context_entries.rb`); discovery: `projects_list` (`project_entries.rb`), `recordings_list` (`recording_entries.rb`), `episodes_list` (`search_entries.rb`); upload: `upload_link_create` (`recording_entries.rb`); requested exact handoff: `workspace_open` (`workspace_entries.rb`); narrow docs: `list_docs`, `search_docs`, `read_doc` (`docs_entries.rb`) |
